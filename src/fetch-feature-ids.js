...
const SOURCES = {
  ...
  webdx: 'https://raw.githubusercontent.com/chromium/chromium/main/third_party/blink/public/mojom/use_counter/metrics/webdx_feature.mojom',
};
const OUT = {
  blink: 'feature-map.js',
  webdx: 'webdx-map.js',
}
...
async function fetchFeatureMap(url) {
  ...
  for (const {groups} of text.matchAll(/\bk(?<feature>\w+)/g)) {
    if (!features.includes(groups.feature) &&
        ...
        !groups.feature.startsWith('DRAFT_')) {
...
async function writeFeatureMap() {
  ...
  const output = path.join(import.meta.dirname, OUT.blink);
  await writeFile(output, stringified);
}

async function writeWebdxMap() {
  const webdxMap = await fetchFeatureMap(SOURCES.webdx);
  const featureStrIter = webdxMap.entries().map(([id, feature]) => `  '${id}': '${feature}',\n`);
  ...
  const output = path.join(import.meta.dirname, OUT.webdx);
  await writeFile(output, stringified);
}

async function run() {
  await writeFeatureMap();
  await writeWebdxMap();
}
